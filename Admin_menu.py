...
class Admin:
    ...
    def admin_mainmenu(self, a, b):
        ...
        mi = (
            PhotoImage(file="images/accounts.png")
            .subsample(a, b)
        ...
        self.tableframe1 = LabelFrame(
            self.mainw, width=350, height=700, bg="#12171c", fg="white"
        )
        self.tableframe1.place(x=1200, y=315, anchor=NE)
        self.tableframe1info = self.tableframe1.place_info()
        self.tableframe = LabelFrame(
            self.mainw, width=350, height=700, bg="#12171c", fg="white"
        )
        ...
        self.printbut = Button(
            ...
            bg="#0c549c",
            fg="white",
            command=self.generate_sales_pdf,
        )
    ...
    def buildprodtable(self):
        ...
        Entry(
            self.formframe,
            textvariable=self.itemeditv,
            font="Montserrat 13",
            bg="#FFFFFF",
            width=20,
        ).place(x=142, y=0, height=30)
        Entry(
            self.formframe,
            textvariable=self.itemeditdescv,
            font="Montserrat 13 ",
            bg="#FFFFFF",
            width=20,
        ).place(x=142, y=60, height=30)
        x = myentry(
            self.formframe,
            textvariable=self.itemeditcatv,
            font="Montserrat 13 ",
            bg="#FFFFFF",
            width=20,
        )
        ...
        Entry(
            self.formframe,
            textvariable=self.itemeditpricev,
            font="Montserrat 13",
            bg="#FFFFFF",
            width=20,
        ).place(x=142, y=180, height=30)
        Entry(
            self.formframe,
            textvariable=self.itemeditstockv,
            font="Montserrat 13",
            bg="#FFFFFF",
            width=20,
        ).place(x=142, y=240, height=30)
        Entry(
            self.formframe,
            textvariable=self.addstock,
            font="Montserrat 13",
            bg="#FFFFFF",
            width=20,
        ).place(x=142, y=300, height=30)
        Button(
            self.formframe,
            text="Update",
    ...
    def changeprodtable(self):
        ...
        if len(li) == 6:
            if self.itemeditv.get() == "" or self.itemeditdescv.get() == "":
                ...
                self.itemeditcatv.get() == ""
                or self.itemeditstockv.get() == ""
                or self.itemeditpricev.get() == ""
            ):
                messagebox.showerror("Error", "Please Fill All Fields")
                return
            else:
            ...
            self.cur.execute(
                ...
                (
                    self.itemeditv.get(),
                    self.itemeditdescv.get(),
                    self.itemeditcatv.get(),
                    int(self.itemeditpricev.get()),
                    (int(self.itemeditstockv.get()) + int(self.addstock.get())),
                    li[0],
                ),
            )
            self.base.commit()
    ...
    def delproduct(self):
        ...
        if (
            messagebox.askyesno(
                "Alert!", "Do you want to remove product from inventory?"
            )
            == True
            and len(li) == 6
        ):
    ...
    def additems(self):
        ...
        Entry(
            self.itemframe,
            width=40,
            textvariable=self.newitemcode,
            font="Montserrat 14",
            bg="white",
        ).grid(row=0, column=1, pady=15, padx=10, ipady=3)
        Entry(
            self.itemframe,
            width=40,
            textvariable=self.newitem,
            font="Montserrat 14",
            bg="white",
        ).grid(row=1, column=1, pady=15, padx=10, ipady=3)
        Entry(
            self.itemframe,
            width=40,
            textvariable=self.newitemdesc,
            font="Montserrat 14",
            bg="white",
        ).grid(row=2, column=1, pady=15, padx=8, ipady=3)
        cat = myentry(
            self.itemframe,
            width=40,
            textvariable=self.newitemcat,
            font="Montserrat 14",
            bg="white",
        )
        ...
        Entry(
            ...
            font="Montserrat 14",
            bg="white",
        ).grid(row=4, column=1, pady=15, padx=10, ipady=3)
        Entry(
            self.itemframe,
            width=40,
            textvariable=self.newitemstock,
            font="Montserrat 14",
            bg="white",
        ).grid(row=5, column=1, pady=15, padx=10, ipady=3)
        ...
        Button(
            ...
            bd=6,
            font="Montserrat 14 bold",
            command=self.insertitem,
            bg="#0c549c",
            fg="white",
        ).grid(row=6, column=1, pady=10, padx=12, sticky="w", ipadx=10)
        Button(
            self.itemframe,
            text="Back",
            height=1,
            ...
            bg="#0c549c",
            fg="white",
        ).grid(row=6, column=1, padx=16, pady=10, sticky="e", ipadx=10)
    ...
    def insertitem(self):
        ...
        if (
            ...
            or self.newitem.get() == ""
            or self.newitemdesc.get() == ""
        ):
            messagebox.showerror("Error", "Please Fill All Fields")
            return
        elif (
            self.newitemcat.get() == ""
            or self.newitemprice.get() == ""
            or self.newitemstock.get() == ""
        ):
            messagebox.showerror("Error", "Please Fill All Fields")
            return
        else:
    ...
    def buildusertable(self):
        ...
        Entry(
            self.formframe1,
            textvariable=self.usernamedit,
            font="Montserrat 13",
            bg="white",
            width=25,
        ).place(x=162, y=105, height=40)
        Entry(
            self.formframe1,
            textvariable=self.passwordedit,
            font="Montserrat 13",
            bg="white",
            width=25,
        ).place(x=162, y=175, height=40)
        profiles = mycombobox(
            self.formframe1, font="Montserrat 13", width=23, textvariable=self.accedit
        )
    ...
    def adduser(self):
        if (
            self.usernamedit.get() == ""
            or self.passwordedit.get() == ""
            or self.accedit.get() == ""
        ):
            messagebox.showerror("Error", "Please Fill All Fields")
            return
    ...
    def generate_sales_pdf(self):
        ...
        file_name = os.path.join(
            dir_path,
            f"Sales_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf",
        )

        # Define table data and style
        table_data = [
            ["Transaction ID", "Invoice ID", "Product ID", "Quantity", "Date", "Time"]
        ]
        for row in sales_data:
            transID, invoiceID, productID, quantity, date, time = (
                row[0],
                row[1],
                row[2],
                row[3],
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, 0),
                        colors.whitesmoke,
                    ),  # Header row text color (whitesmoke)
                    ...
                    (
                        ...
                        (-1, -1),
                        colors.grey,
                    ),  # Content row background color (grey)
